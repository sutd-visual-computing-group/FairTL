...
def build_80_20_unbalanced_datasets_clf_celeba(dataset_name, split, perc=1.0):
	"""
...
def build_60_40_unbalanced_datasets_clf_celeba(dataset_name, split, perc=1.0):
	"""
	Builds (80-20) and (50-50) biased/unbiased dataset splits.
	...
	if split == 'train': 
	    # adjust proportions of unbalanced_indices
	    new_females = females[(n_balanced//2):-(6750*multiplier)]
	    # additional_males
	    additional_males = np.where(labels[:,20]==1)[0][40504:]
	    new_males = np.hstack(
	    	(males[(n_balanced//2):], additional_males[0:(6750*multiplier)]))
	elif split == 'val':
	    # adjust proportions of unbalanced_indices
	    new_females = females[(n_balanced//2):-815*multiplier]
	    additional_males = np.where(labels[:,20]==1)[0][4889:]
	    new_males = np.hstack(
	    	(males[(n_balanced//2):], additional_males[0:815*multiplier]))
...
def build_75_25_unbalanced_datasets_clf_celeba(dataset_name, split, perc=1.0):
	"""
	Builds (80-20) and (50-50) biased/unbiased dataset splits.
	...
	if split == 'train': 
	    # adjust proportions of unbalanced_indices
	    new_females = females[(n_balanced//2):-int(6750*multiplier)]
	    # additional_males
	    additional_males = np.where(labels[:,20]==1)[0][40504:]
	    new_males = np.hstack(
	    	(males[(n_balanced//2):], additional_males[0:int(6750*multiplier)]))
	elif split == 'val':
	    # adjust proportions of unbalanced_indices
	    new_females = females[(n_balanced//2):-int(815*multiplier)]
	    additional_males = np.where(labels[:,20]==1)[0][4889:]
	    new_males = np.hstack(
	    	(males[(n_balanced//2):], additional_males[0:int(815*multiplier)]))
